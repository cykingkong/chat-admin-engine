<template>
  <div v-if="show" class="mark">
    <div class="midBox">
      <img
        src="https://img.adyinqing.com/upai_shop/n2w5hwvp1688608894156"
        alt=""
        class="modal_img"
      />
      <img
        src="https://img.adyinqing.com/upai_shop/r9i0fvvi1688609365805"
        alt=""
        class="close_icon"
        @click="closeDialog"
      />
...
if (
  Date.now() < dayjs(userStore.$state.memberExpiredAt).valueOf() &&
  userStore.$state.expireReminder !== 1 &&
  ...
  const titleNode1 = h('span', '您的魔力智投账号使用有效期不足');
  const titleNode2 = h(
    'span',
  ...
  const space = h(
    ...
    [titleNode1, titleNode2, titleNode3]
  );
  const contentNode = h(
    'div',
    {
  ...
  Modal.info({
    ...
    async onClose() {
      await changeGuidance({ expireReminder: 1 });
    },
  });
}
