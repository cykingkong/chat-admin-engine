<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-row class="banner" justify="space-between">
      <a-col flex="250px" class="accountCheck">
        <div class="title"> 阶段数据 </div>
      </a-col>
    </a-row>
    <div style="padding: 0 15px 0 0">
      <a-empty v-if="!dataGrid.length" />

      <Chart
        v-else
        style="height: 328px; margin-top: 10px"
        :option="chartOption"
      />
    </div>
    <!-- </a-card> -->
...
<script lang="ts" setup>
  ...
  import { reportHourData, reportDayData, plantStageGrid } from '@/api/plant';
      ...
      xAxis: {
        ...
        axisLabel: {
          ...
          formatter(value: number, idx: number) {
            if (idx === 0 || idx === xAxis.value.length - 1) return `${value}`;
            return '';
          },
        },
        axisLine: {
  ...
  const fetchData = async (req: any) => {
    ...
    try {
      ...
      const { data } = await plantStageGrid(req);
      if (data.grid) {
        dataGrid.value = data.grid;
        data.grid.forEach((el: any, key: any) => {
          if (key === 0) {
            ...
            xAxis.value.push(`${dayjs(el.createdAt).format('HH:mm:ss')}`);
          } else {
            costData.value.push(el.cost || 0);
            showData.value.push(el.productOrderCount || 0);
            clickData.value.push(el.productPlaceorderGmv || 0);
      ...
      console.log(
        costData.value,
        xAxis.value.length,
        xAxis.value[xAxis.value.length - 1]
      );
    }
  };
