                    ...
                    <a-space>
                      ...
                      <a-space direction="vertical" fill>
                        ...
                        <div
                          ...
                          >
                            {{ record.videoInfo.videoNickName || '-' }}
...
<script lang="ts" setup>
  ...
  import { sphInfo, getOrderDetail } from '@/api/sph';
  ...
  const fetchData = async (params: any) => {
    setLoading(true);
    try {
      const { data } = await taskGrid(params);
  ...
  const handleClickDelete = async (row: any) => {
    const { data, code } = await taskDeleted({ taskId: row.taskId });
    if (code === 200) {
  ...
  const createdGetPlant = async () => {
    ...
    console.log(
      overPlantNum === createTaskOrderInfo.value.total,
      'overPlantNum === createTaskPayOrderList.value.length'
    );
    if (overPlantNum === createTaskOrderInfo.value.total) {
